...
```{r}
m.dn <- round(dist.dna(dn, as.matrix = T, model = 'raw', pairwise.deletion = T), 4) %>% as_tibble(rownames = NA) %>% 
  rownames_to_column("ID") %>% 
  write_csv("cytb_distances_raw.csv")
```
...
mat2[upper.tri(mat2)] <- ""
mat2 <- mat2 %>% as.data.frame.matrix() %>% as_tibble(rownames = NA) %>% write.csv('distance_matrix_raw.csv')
```
...
d <- read_csv('distance_matrix_range_newline_raw.csv') %>% column_to_rownames(var = '...1')
...
d %>% kbl(caption = "Raw cytb Differences   mean (min, max)", longtable = T, escape = F, align = 'c') %>% 
  kable_classic(#full_width = F, 
               html_font = "Cambria", 
               font_size = 10, 
               position = 'center') %>% 
 #column_spec(position = 'center')
  save_kable(file = "cytb_differences_raw.png",zoom = 2)
```
